...
public class SK02_10_225150701111036_SyahrezaFistiFerdian {
    /*  
        Ketua :
        ...
        Anggota 1 :
            Nama    : Jerico Willy Raynaldy
            NIM     : 225150707111019

        Anggota 2 :
            Nama    : NAURA ISTITAH MUHTASYAM
            NIM     : 225150701111021
     */
            ...
            switch(menuChoice) {
                case 1 : 
                    if(!data.isEmpty()) {
                        ...
                        char inputChoice = ' ';
                        while(!stopLoop) {
                            System.out.print("Apakah Anda ingin meng-override data riwayat pasien tersebut? (Y/N): ");
                            
                            try {
                                inputChoice = input.nextLine().toLowerCase().charAt(0);
                                if(!(inputChoice == 'y' || inputChoice == 'n')) throw new Exception();
                                stopLoop = true;
                            } catch (Exception e) {
                ...
                case 3 : 
                    ...
                    try {
                        ...
                        if(!(confirm == 'y' || confirm == 'n')) throw new Exception();
    ...
    public static void undo() {
        if(!data.isEmpty()) {
            String undoElement = data.pop();
            undoRedoHistory.push(undoElement);
        }
        System.out.printf("%sTeks preview: %s%s\n", ANSI_CYAN_BOLD, data, ANSI_RESET);
    }
    ...
    public static void redo() {
        if(!undoRedoHistory.isEmpty()) {
            String redoElement = undoRedoHistory.pop();
            data.push(redoElement);
        }
        System.out.printf("%sTeks preview: %s%s\n", ANSI_CYAN_BOLD, data, ANSI_RESET);
    }
